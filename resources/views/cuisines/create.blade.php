...
@section('title', 'Nova Cozinha')
@section('header', 'Nova Cozinha')
@section('subheader', 'Adicione um novo tipo de cozinha')

@section('content')
    <div class="max-w-3xl mx-auto">
        <form action="{{ route('cuisines.store') }}" method="POST" class="space-y-6">
            @csrf
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <textarea name="description" id="description" rows="4" required
                                          ...
                                          placeholder="Descreva a cozinha...">{{ old('description') }}</textarea>
                        ...
                        <div>
                            ...
                            <div class="flex items-center space-x-4">
                                <div class="w-12 h-12 bg-primary/10 rounded-lg flex items-center justify-center group-hover:bg-primary/20 transition-colors">
                                    <i class="{{ old('icon', 'fas fa-utensils') }} text-2xl text-primary" id="icon-preview"></i>
                                </div>
                                <div class="flex-1">
                                    <div class="relative">
                                        <input type="text" name="icon" id="icon" value="{{ old('icon', 'fas fa-utensils') }}" required
            ...
            <div class="flex justify-end space-x-4">
                <a href="{{ route('cuisines.index') }}" class="btn btn-secondary">
                    <i class="fas fa-times mr-2"></i> Cancelar
                </a>
                <button type="submit" class="btn btn-primary">
                    <i class="fas fa-plus mr-2"></i> Criar Cozinha
                </button>
            </div>
        </form>
    </div>
